...
namespace SecureBootWatcher.Client.Services
{
   ...
   public Task<SecureBootCertificateCollection> EnumerateAsync(CancellationToken cancellationToken)
 {
  ...
  return null;
        ...
        private void EnumerateDatabaseCertificates(
            SecureBootCertificateCollection collection,
            string databaseName,
    ...
    var possiblePaths = new[]
         {
         $"{SecureBootBasePath}\\{databaseName}",
             $"{SecureBootBasePath}\\Platform\\{databaseName}",
             $"SYSTEM\\CurrentControlSet\\Control\\Cryptography\\Configuration\\Local\\SSL\\00010002\\{databaseName}"
      };
