...
namespace WindowsVersionsCore.Services
{
    ...
    public class WindowsService : IWindowsService
    {
            ...
            IConfiguration configuration)
        {
            ...
            // Blob container name from configuration (Storage:ContainerName), default to "windowsversions"
            _containerName = _configuration["Storage:ContainerName"] ?? "windowsversions";
        ...
        public async Task<DateTime?> GetLastUpdateTimeAsync()
        {
            try
            {
                ...
                var metadataBlob = container.GetBlobClient("last-update.json");

                if (await metadataBlob.ExistsAsync())
                {
                    var content = await metadataBlob.DownloadContentAsync();
                    var metadata = JsonSerializer.Deserialize<Dictionary<string, object>>(content.Value.Content.ToString());
        ...
        private async Task SaveUpdatesToStorageAsync(WindowsEdition edition, List<WindowsUpdate> updates)
        {
            try
            {
                ...
                var blobName = $"{edition.ToString().ToLower()}-updates.json";
                ...
                var localPath = Path.Combine(AppContext.BaseDirectory, $"{edition.ToString().ToLower()}-updates-latest.json");
                await File.WriteAllTextAsync(localPath, json);
                _logger.LogInformation("Saved {Count} updates for {Edition} to storage and local copy {Path}", updates.Count, edition, localPath);
            }
            catch (Exception ex)
            {
        ...
        private async Task SaveReleaseVersionsToStorageAsync(WindowsEdition edition, WindowsReleaseVersions releaseVersions)
        {
            try
            {
                ...
                var blobName = $"{edition.ToString().ToLower()}-versions.json"; // keep same name for compatibility
                var blob = container.GetBlobClient(blobName);

                var json = JsonSerializer.Serialize(releaseVersions, new JsonSerializerOptions { WriteIndented = true });
                await blob.UploadAsync(BinaryData.FromString(json), overwrite: true);

                var localPath = Path.Combine(AppContext.BaseDirectory, $"{edition.ToString().ToLower()}-versions-latest.json");
                await File.WriteAllTextAsync(localPath, json);

                await UpdateLastUpdateTimeAsync();
                _logger.LogInformation("Saved channel-specific versions for {Edition} to storage and local copy {Path}", edition, localPath);
            }
            catch (Exception ex)
            {
        ...
        private async Task UpdateLastUpdateTimeAsync()
        {
            try
            {
                ...
                await metadataBlob.UploadAsync(BinaryData.FromString(json), overwrite: true);
            }
            catch (Exception ex)
            {
        ...
        private async Task<WindowsReleaseVersions?> LoadReleaseVersionsFromStorageAsync(WindowsEdition edition)
        {
            ...
            {
                var container = GetContainer();
                var blobName = $"{edition.ToString().ToLower()}-versions.json";
                var blob = container.GetBlobClient(blobName);
        ...
        private async Task<List<WindowsFeatureUpdate>?> LoadFeatureUpdatesFromStorageAsync(WindowsEdition edition)
        {
            ...
            {
                var container = GetContainer();
                var blobName = $"{edition.ToString().ToLower()}-feature-updates.json";
                var blob = container.GetBlobClient(blobName);
        ...
        private async Task<List<WindowsVersion>?> LoadVersionsFromStorageAsync(WindowsEdition edition)
        {
            try
            {
                ...
                if (await blob.ExistsAsync())
                {
                    var content = await blob.DownloadContentAsync();
                    var json = content.Value.Content.ToString();
                    return JsonSerializer.Deserialize<List<WindowsVersion>>(json);
                }
            }
            catch (Exception ex)
            {
